...
# The word popup on index-word-N.html. Its link text holds the signs of the
# word, joined by "-", and the popup ends with "(N signs)". SigLA titles the
# link "View othe rattestations of this sequence" (sic) on most pages, and
# "View attestations of this sequence" on some (seen on HT 129 and HT 130,
# 2026-09-18).
_WORD_POPUP = re.compile(
    r'class="word popup[^"]*"\s+id="word-(\d+)">.*?'
    r'<a title="View (?:othe r)?attestations of this sequence"[^>]*>(.*?)</a>'
    r'\s*\((\d+) signs?\)',
    re.DOTALL,
)
_WORD_TOKEN = re.compile(
    r'<span class="(sure-reading|unsure-reading|unclassified)">(.*?)</span>'
    r'|(\[\?\])'
    r'|([-\[\]])'
)

# Marks used in a word string (words in corpus.json, reading in
# corpus_structured.json):
#   ra2        a sure sign; a SigLA subscript is kept as a digit (ra<sub>2</sub>)
#   re?        an unsure sign: SigLA marks the reading "unsure-reading"
#   [?]        a sign that SigLA shows but does not identify
#   [unclassified]  a sign that SigLA labels unclassified
# A SigLA superscript variant letter (qi<sup>f</sup>) names a variant form of
# the same sign, so the word string keeps "qi". The damage brackets "[" and
# "]" that SigLA prints at a broken word edge are not signs and are not kept.
UNSURE_MARK = "?"


def _sign_text(inner: str) -> str:
    inner = re.sub(r"<sub>(\d+)</sub>", r"\1", inner)
    inner = re.sub(r"<sup>[^<]*</sup>", "", inner)
    if "<" in inner:
        raise ValueError(f"unexpected markup in sign: {inner!r}")
    return inner.strip()


def parse_word_page(html: str) -> dict | None:
    """Parse one SigLA index-word-N.html page.

    Returns {"index": N, "tokens": [(text, certain), ...], "reading": str,
    "sign_count": int} or None when the page has no word popup. The number of
    tokens always equals SigLA's own "(N signs)"; a mismatch raises.
    """
    m = _WORD_POPUP.search(html)
    if not m:
        return None
    body, declared = m.group(2), int(m.group(3))
    tokens = []
    pos = 0
    for t in _WORD_TOKEN.finditer(body):
        gap = body[pos:t.start()].strip()
        if gap:
            raise ValueError(f"unparsed text in word page: {gap!r}")
        pos = t.end()
        kind, inner, unknown, punct = t.groups()
        if punct:
            continue
        if unknown:
            tokens.append(("[?]", False))
        elif kind == "sure-reading":
            tokens.append((_sign_text(inner), True))
        elif kind == "unsure-reading":
            tokens.append((_sign_text(inner) + UNSURE_MARK, False))
        else:
            tokens.append(("[unclassified]", False))
    if body[pos:].strip():
        raise ValueError(f"unparsed text in word page: {body[pos:]!r}")
    if len(tokens) != declared:
        raise ValueError(f"word page declares {declared} signs, parsed {len(tokens)}")
    return {
        "index": int(m.group(1)),
        "tokens": tokens,
        "reading": "-".join(t for t, _ in tokens),
        "sign_count": declared,
    }
...
def parse_document_html(doc_dir: str, doc_name: str) -> Document | None:
    ...
    # Each word has its own page, named index-word-N.html. The document also
    # has an overview page named index-word.html, which repeats every sign of
    # every word. A filter of "index-word" matches the overview page too, and
    # the overview page then becomes one pseudo-word equal to the join of all
    # the real words. The filter must require the hyphen and the number.
    doc_path = os.path.join(doc_dir, doc_name)
    word_page_pattern = re.compile(r'^index-word-(\d+)\.html$')
    word_pages = []
    for f in os.listdir(doc_path):
        m = word_page_pattern.match(f)
        if m:
            word_pages.append((int(m.group(1)), f))
    word_files = [f for _, f in sorted(word_pages)]
    ...
    for i, wf in enumerate(word_files):
        ...
        # Every word page yields a word. An earlier version kept only the
        # sure-reading spans, so a word page with no sure-reading span was
        # dropped, unsure signs vanished from inside words, and a subscript
        # such as ra<sub>2</sub> was cut to "ra". See parse_word_page.
        parsed = parse_word_page(whtml)
        if parsed is None:
            raise ValueError(f"{doc_name}/{wf}: no word popup found")
        signs = []
        for j, (token, certain) in enumerate(parsed["tokens"]):
            signs.append(SignOccurrence(
                occurrence_number=j,
                sign_type=None,
                role=SignRole.SYLLABOGRAM,
                reading=token,
                reading_certain=certain,
            ))
